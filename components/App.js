...
function Logo() {
  return html`<a href="/index.html" id="logo">
...
export default class App extends Component {
  constructor(props) {
    ...
    this.state = isServer ? this.props : window.state;
    // From initial state: { [url]: {…props} }
    debugger;
    this.state.currentURL = Object.keys(props)[0];
  }
  ...
  render(props) {
    ...
    return html`
      ...
      <main id="main">
        ...
        <${Router}
          url=${currentURL}
          onChange=${isServer ? null : this.handleRoute}
        >
          ${routes.map(route => {
            return route.component
              ...
              />`
              // FIXME: possibly I should do `...${route}` (and let component
              // down the tree render the route handling component) ?
              : html`<${AsyncRoute}
